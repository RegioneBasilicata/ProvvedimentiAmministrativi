...
Public Class svrDisposizioniBase
    ...
    Overrides Function Elabora(ByVal cFunzione As Integer, ByVal vparm As Object, Optional ByVal controllo As Integer = 0, Optional ByVal FlagChiudiTutte As Boolean = True, Optional ByVal destinatarioInoltro As Integer = -1) As Object
        ...
        Select Case cFunzione
            ...
            Case Dic_FODocumentale.cfo_Crea_Disposizione '34
                Elabora = FO_Crea_Disposizione(vparm(1))
            Case Dic_FODocumentale.cfo_Passo_Disposizione '15
                Elabora = FO_Passo_Disposizione(vparm(1), destinatarioInoltro)
            Case Else
...
FineSub:
    ...
    Friend Overridable Function FO_Passo_Disposizione(ByVal vParm As Object, Optional ByVal destinatarioInoltro As Integer = -1) As Object
        Const SFunzione As String = "FO_Passo_Disposizione"
        ...
        vRitPar(0) = 0
        vRitPar(1) = ""
         Log.Info("***FINE FO_Passo_Disposizione - idDocumento: " & vParm(Dic_FODocumentale.vc_Passo_Determina.c_idDocumento) & " azione: " & vParm(Dic_FODocumentale.vc_Passo_Determina.c_azione) & " " & Now)
        ...
        Sqlq = "SELECT     Awf_attivita, Awf_nomeAttivita, Awf_nomeOggetto, Awf_dataBase , Awf_parametri " & _
               ...
               " WHERE    (Awf_flusso = 'DISPOSIZIONI' )  " & _
        ...
        If AggiornaPassoDisposizione(vRegAtt, azione, idIstanzaWFE, idWorkitem, pNodoFlussoDetermina, pNodoFlussoUfficio, attore, pDirD, pDirU, pProssimoAttore, suggerimento, flagUrgente) Then
...
Herr:
    ...
    Friend Overridable Function FO_Crea_Disposizione(ByVal vParm As Object) As Object
        Const SFunzione As String = "FO_Crea_Disposizione"
        ...
        codUfficioProponente = vParm(Dic_FODocumentale.vc_Crea_Disposizione.c_cod_ufficio_proponente)
        utenteCreazione = vParm(Dic_FODocumentale.vc_Crea_Disposizione.c_utente_creazione) & ""
        If IsDate(vParm(Dic_FODocumentale.vc_Crea_Disposizione.c_data_creazione)) Then
            dataCreazione = CDate(vParm(Dic_FODocumentale.vc_Crea_Disposizione.c_data_creazione))
        Else
        ...
        Sqlq = "SELECT     Sta_id, Sta_attributo, Sta_valore " & _
               ...
               " WHERE       (Sta_procedura = 'DISPOSIZIONI' or Sta_procedura = '*') AND (Sta_attributo = 'UFFICIO_PROPONENTE') " & _
        ...
        If vR(0) <> 0 Then
            vRitPar(0) = 1
            vRitPar(1) = "L'ufficio non è abilitato alla creazione delle disposizioni"
            GoTo FineSub
        End If
        ...
        Sqlq = "(SELECT     TAB_Operatori_Attributi.Toa_Valore " & _
               ...
               " and ( TOA_Procedura = 'DISPOSIZIONI'  or  TOA_Procedura= '*' ) ) " & _
               " UNION " & _
               ...
               " and ( TOA_Procedura = 'DISPOSIZIONI'  or  TOA_Procedura= '*' ) ) "
        ...
        vR = Calcola_Progressivo(DB, "NPDS", Year(Now), False, codUfficioProponente)
